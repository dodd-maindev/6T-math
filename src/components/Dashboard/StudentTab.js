...
import { UserPlus, Search, ChevronRight, GraduationCap } from 'lucide-react';

/**
 * Responsive full-width student management table with search and creation dialog.
 */
export const StudentTab = ({ classroom }) => {
  ...
  if (activeSubmission) {
    return <Scorecard submission={activeSubmission} studentName={activeStudent.email} onClose={() => setActiveSubmission(null)} />;
  }

  if (activeStudent) {
    return <StudentProfileView student={activeStudent} submissions={submissions} onBack={() => setActiveStudent(null)} onSelectSubmission={setActiveSubmission} />;
  }

  const filtered = students.filter(s => s.email.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 sm:gap-0">
        ...
        <div className="flex items-center space-x-2 sm:space-x-3 w-full sm:w-auto">
          <div className="relative flex-1 sm:flex-initial">
            ...
            <input type="text" placeholder="Tìm học sinh..." className="w-full sm:w-48 pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-xl text-xs text-slate-800 focus:outline-none focus:border-purple-500 shadow-sm" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          </div>
            ...
            filtered.map((std, idx) => (
              <div key={std.id} onClick={() => { setActiveStudent(std); fetchSubmissions(std.id); }} className="px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between hover:bg-purple-50/40 cursor-pointer transition-colors group">
                <div className="flex items-center space-x-3">
                  ...
                  <div className="min-w-0"><p className="text-xs font-bold text-slate-800 group-hover:text-purple-900 truncate max-w-[180px] sm:max-w-md">{std.email}</p><p className="text-[10px] sm:text-[11px] text-slate-400">Tham gia: {new Date(std.created_at).toLocaleDateString('vi-VN')}</p></div>
                </div>
                <div className="flex items-center space-x-1 sm:space-x-2 text-xs font-bold text-purple-600 shrink-0"><span className="hidden sm:inline">Xem hồ sơ</span><ChevronRight className="w-4 h-4" /></div>
              </div>
            ))
          )}
        </div>
